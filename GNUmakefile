...
#
SUBDIRS := ZigZag  HitMaker OverlapEvents MakeWFTTree PMTCommissioning Paddles 3FEM HitCalibration BeamTiming #ADD_NEW_SUBDIR ... do not remove this comment from this line
...
all:
	@for i in $(SUBDIRS); do ( echo "" && echo "Compiling $$i..." && cd $(LARLITE_USERDEVDIR)/Commissioning/$$i && $(MAKE) ) || exit $$?; done
#####################################################################################
#
# CLEANs...
#
clean:
	@for i in $(SUBDIRS); do ( echo "" && echo "Cleaning $$i..." && cd $(LARLITE_USERDEVDIR)/Commissioning/$$i && $(MAKE) clean && rm -f $(LARLITE_LIBDIR)/$$i.* ) || exit $$?; done
...
doxygen:
	...
	@mkdir -p $(LARLITE_USERDEVDIR)/Commissioning/doc/dOxygenMyProject
	@doxygen $(LARLITE_USERDEVDIR)/Commissioning/doc/doxygenMyProject.script
